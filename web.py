...
if 'prompt_counter' not in st.session_state:  # Счётчик для смены ключа
...
if st.button("Отчистить и загрузить заново"):
    ...
    st.session_state.prompt_counter += 1  # Меняем ключ
    st.rerun()
    ...
    with st.spinner("AI agent обрабатывает данные..."):
        try:
            ...
            if response is False:
                ...
                st.session_state.response = "default response"
